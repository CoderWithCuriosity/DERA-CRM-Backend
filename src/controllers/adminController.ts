...
import { 
  User, Contact, Deal, Ticket, Campaign, AuditLog, Organization,
  sequelize 
} from '../models';
import { 
  HTTP_STATUS, USER_ROLES, AUDIT_ACTIONS, ENTITY_TYPES, TIME 
...
export const getBackupStatus = catchAsync(async (req: Request, res: Response) => {
  ...
  const status = await getBackupStatus(backup_id);
