...
export const getContacts = catchAsync(async (req: Request, res: Response) => {
  ...
  if (tag) {
    whereCl clause.tags = { [Op.contains]: [tag] };
  }
  ...
  const order = [];
  if (sort_by) {
    order.push([sort_by as string, (sort_order as string) || 'DESC']);
  } else {
  ...
  const enhancedContacts = contacts.rows.map(contact => {
    const contactData = contact.toJSON();
    return {
      ...contactData,
      deals_count: contactData.deals?.length || 0,
      tickets_count: contactData.tickets?.length || 0,
      last_activity: contactData.activities?.[0]?.created_at || null
    };
  });
...
export const exportContacts = catchAsync(async (req: Request, res: Response) => {
  ...
  const exportFields = fields ? (fields as string).split(',') : null;

  const exportData = await generateExport(contacts, format as string, exportFields);
